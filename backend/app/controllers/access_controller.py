    ...
    def sign_in(self, user):
        ...
        if cognito_result is not False:
            ...
            response.set_cookie(key="refreshToken",
                                ...
                                secure=True,
                                httponly=True,
                                domain=".api.gm.perfect-projects.link" if user.get("username") == "resanter" else ".api.grind-meter.com", # TODO fix this after buying normal domain
                                expires=int(datetime.now().timestamp() + g.REFRESH_TOKEN_EXP_MIN * 60))
            response.set_cookie(key="username",
                                value=user.get("username"),
                                samesite="none",
                                secure=True,
                                domain=".gm.perfect-projects.link" if user.get("username") == "resanter" else ".grind-meter.com", # TODO fix this after buying normal domain
                                expires=int(datetime.now().timestamp() + g.REFRESH_TOKEN_EXP_MIN * 60))
            return response
